...
class AGENT:
    ...
    def Actor(self):                     
        ...
        lidar_conv = Conv1D(self.layers[0],4, padding='valid', activation='relu', kernel_regularizer=self.k_r)(lidar_input)
    ...
    def TRAIN(self, batch, count):
        if len(self.deck) < batch:
            # number of steps batch
            sample_indx = random.sample(self.deck, count)
        else:
            # Random Batch sample
...
if __name__ == '__main__':
    ...
    tqdm_run=tqdm(range(EPISODES), desc='Score', leave=True, unit='episode')    
    for i in tqdm_run: 
        ...
        while True:            
            ...
            training_time = agent.TRAIN(BATCH, counter)
            ...
            # END_IF
        # END_WHILE
        tqdm_run.set_description("Score: " + str(ep_rew_total))
        tqdm_run.refresh()
